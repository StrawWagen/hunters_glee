...
-- Returns nil if the leaderboard can be shown, or a string reason to show as overlay instead
local function getLeaderboardBlockReason()
    local ply = LocalPlayer()
    if not IsValid( ply ) then return "Player not ready" end

    -- If round is active and player is alive, we block the leaderboard (overlay instead)
    local roundActive = GAMEMODE.RoundState and GAMEMODE:RoundState() == GAMEMODE.ROUND_ACTIVE
    local alive = ply:Alive()
    if roundActive and alive then
        return "Focus on the hunt, you could be killed at any time."
    end
...
function bankTop:Create( container )
    ...
    local hudPadding = GAMEMODE.shopStandards.borderPadding * scale
    ...
    function listPanel:Paint( w, h )
        ...
        -- Check if we're supposed to show an overlay instead of the leaderboard
        local blockReason = getLeaderboardBlockReason()

        -- If blocked, render the overlay message and bail before drawing entries
        if blockReason then
            -- Title remains. Draw an explanatory message centered.
            local msg = blockReason
            surface.SetFont( "termhuntShopItemFont" )
            local _, th = surface.GetTextSize( msg )
            draw.SimpleText( msg, "termhuntShopItemFont", w * 0.5, padding + titleH + detailGap + th, GAMEMODE.shopStandards.white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )

            -- If we have a bank percent available and a hovered entry would normally show it, add a subtle hint row
            local hint = "Only the dead have time for banking."
            surface.SetFont( "termhuntShopItemSmallerFont" )
            draw.SimpleText( hint, "termhuntShopItemSmallerFont", w * 0.5, padding + titleH + detailGap + th * 2 + 6, GAMEMODE.shopStandards.white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
            return
        end

        -- Precompute mouse position (only needed when we actually draw entries)
        self.hoverIndex = nil
        ...
        for i = 1, count do
            ...
            if self.playerIndex and i == self.playerIndex then
                local splash = "You!"
                local bounce = math.sin( ( CurTime() * 5 ) + ( self._lastScrollSpeed / 25 ) ) * 2
        ...
        if idx then
            ...
            if minFunds > 0 and funds < minFunds then
                ...
                local periodText = asDays .. " day" .. sOrNoS

                local loss = math.floor( funds * percent / 100 + 0.5 )
                local lossText = string.format( "Losing %s score every %s!", formatFunds( loss ), periodText )
