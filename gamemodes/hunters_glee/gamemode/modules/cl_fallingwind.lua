...
hook.Add( "Think", "glee_Think_fallingwind", function()
    ...
    elseif IsValid( obsTarg ) and ( myObserverMode == OBS_MODE_CHASE or myObserverMode == OBS_MODE_IN_EYE ) then
        velocity = getVelocityBulletproof( obsTarg, obsTarg:GetTable() )

    elseif myObserverMode == OBS_MODE_ROAMING then
        velocity = me:GetVelocity() * 0.6 -- not so bulletproof afterall
    ...
    if IsValid( obsTarg ) and obsTarg:IsNextBot() then -- the jitters
        avgCount = 200
