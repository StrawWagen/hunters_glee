...
local function DrawDeath( x, y, death, time )
	...
	elseif killicon.Draw then
		killicon.Draw( x, y + y / 2, death.icon, alpha )
	...
	if ( death.right ) then
		...
		if doOverride then
			right = rightOverride
			rightColor = rightColorOverride

		end

		draw.SimpleText( right, "ChatFont", x + ( w / 2 ) + 16, y + h / 2, rightColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
